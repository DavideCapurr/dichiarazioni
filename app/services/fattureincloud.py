...
def _get_api_client() -> fattureincloud_python_sdk.ApiClient:
    ...
    token = settings.fic_access_token
    company_id = settings.fic_company_id
    
    # Mostra solo i primi 5 caratteri del token per sicurezza, o un avviso se è vuoto
    token_preview = f"{token[:5]}..." if token else "NESSUN_TOKEN_TROVATO (VUOTO!)"
    logger.info(f"DEBUG API - Company ID: {company_id}")
    logger.info(f"DEBUG API - Token: {token_preview}")
    # -----------------------------
...
def get_client(client_id: int) -> ClientInfo:
    ...
    return _entity_to_client_info(response.data)    return fattureincloud_python_sdk.ApiClient(configuration)
...
def get_client(client_id: int) -> ClientInfo:
    ...
    return _entity_to_client_info(response.data)        phone=entity.phone,
        address_street=entity.address_street,
        address_postal_code=entity.address_postal_code,
        address_city=entity.address_city,
        address_province=entity.address_province,
